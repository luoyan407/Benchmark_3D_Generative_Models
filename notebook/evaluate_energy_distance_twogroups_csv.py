...
def compute_metrics_pair(p1, p2, threshold=0.01, resolution=64, max_emd_points=2048):
    """Computes all 5 metrics for a single pair."""
    results = {}
...
def calculate_energy_distance(group_A, group_B, do_icp=True):
    ...
    for k in keys:
        ...
        print(f"{k.upper():<10} | Energy Dist: {ed_value:.6f}")
...
if __name__ == "__main__":
    ...
    # 1. Setup list to collect results for CSV
    all_results = []
    ...
    if len(data_A_N) > 0 and len(data_B_A) > 0:
        ...
        # Save to list
        if results:
            row = {'Comparison': 'N_vs_A', 'Num_A': len(data_A_N), 'Num_B': len(data_B_A)}
            row.update(results)
            all_results.append(row)
    else:
        print("No data loaded for N vs A.")
    ...
    if len(data_A_N) > 0 and len(data_B_D) > 0:
        ...
        # Save to list
        if results:
            row = {'Comparison': 'N_vs_D', 'Num_A': len(data_A_N), 'Num_B': len(data_B_D)}
            row.update(results)
            all_results.append(row)
    else:
        print("No data loaded for N vs D.")
    ...
    if len(data_A_N) > 0 and len(data_B_G) > 0:
        ...
        # Save to list
        if results:
            row = {'Comparison': 'N_vs_G', 'Num_A': len(data_A_N), 'Num_B': len(data_B_G)}
            row.update(results)
            all_results.append(row)
    else:
        print("No data loaded for N vs G.")

    # ==========================================
    # 5. Write to CSV
    # ==========================================
    if all_results:
        output_csv = os.path.join(os.path.dirname(input_folder), "energy_distance_results.csv")
        fieldnames = ['Comparison', 'Num_A', 'Num_B', 'f1', 'iou', 'dice', 'chamfer', 'emd']
