...
input_img_folder = "/PHShome/yl535/project/python/datasets/Fundus_Image_Vessel_Segmentation/test/Original"
input_mask_folder = "/PHShome/yl535/project/python/datasets/Fundus_Image_Vessel_Segmentation/test/Ground_truth"
...
files = [f for f in os.listdir(input_img_folder) 
         if os.path.isfile(os.path.join(input_img_folder, f))]
...
for filename in os.listdir(input_img_folder):
    if filename.lower().endswith(valid_extensions):
        ...
        input_img = os.path.join(input_img_folder, filename)
        input_mask = os.path.join(input_mask_folder, filename)
        output_ply = os.path.join(output_dir, f"{base_name}.ply")

        # if input_img exists, continue
        ...
        # try:
        #     # Assuming output["gs"] has an underlying tensor wrapper or .xyz attribute
        #     # If strictly using the 'scene_gs' logic:
        #     scene_gs = make_scene(output)
            
        #     # Check if point cloud has points before normalizing
        #     if scene_gs.gaussians.xyz.shape[0] == 0:
        #         print(f"Skipping {filename}: Model generated 0 Gaussian points.")
        #         continue
        ...
        #     print(f"Skipping {filename} during rendering: {e}")
        # except Exception as e:
