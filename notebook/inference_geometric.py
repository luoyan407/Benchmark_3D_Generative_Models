...
# from sam3d_objects.pipeline.inference_pipeline_pointmap_geometric import InferencePipelinePointMapGeometric
from sam3d_objects.model.backbone.tdfy_dit.utils import render_utils

PROJECT_ROOT = Path(__file__).resolve().parent.parent
sys.path.append(os.path.join(str(PROJECT_ROOT), "sam3d_objects", "pipeline"))

# Approach A: training-free geometric sparse structure
from inference_pipeline_pointmap_geometric import InferencePipelinePointMapGeometric
...
class Inference:
    ...
    def __init__(
        ...
        config_file: str,
        compile: bool = False,
        use_geometric_stage1: bool = False,
        geometric_ss_config: Optional[dict] = None,
    ):
        """
        Args:
            config_file: Path to the Hydra YAML config.
            compile: Whether to torch.compile model components.
            use_geometric_stage1: If True, replace Stage 1 diffusion with
                training-free geometric voxelization (Approach A).
            geometric_ss_config: Config dict for GeometricSparseStructure.
                Only used when use_geometric_stage1=True. Defaults:
                    closing_radius=2, closing_iterations=1,
                    surface_thickness=1, silhouette_carving=True
        """
        ...
        self.use_geometric_stage1 = use_geometric_stage1

        if use_geometric_stage1:
            # Inject geometric_ss_config into the hydra config so the
            # subclass constructor receives it
            config["geometric_ss_config"] = geometric_ss_config or {}
            # Override the _target_ to use the geometric subclass
            config["_target_"] = (
                "geometric_sparse_structure.InferencePipelinePointMapGeometric"
            )
            self._pipeline: InferencePipelinePointMapGeometric = instantiate(config)
        else:
...
def normalized_gaussian(scene_gs, in_place=False, outlier_percentile=None):
    ...
    # Retrieve opacity values
    opacity = scene_gs.get_opacity.squeeze()
    
    # Try the original strict threshold
    active_mask = opacity > 0.9

    # If the mask is empty, lower the threshold
    if active_mask.sum() == 0:
        print("Warning: No points with opacity > 0.9 found. Lowering threshold to 0.5.")
        active_mask = opacity > 0.5

    # If still empty, include all points (prevents crash on faint objects)
    if active_mask.sum() == 0:
        print("Warning: Object is highly transparent. Using all points for normalization.")
    ...
    # Prevent division by zero if inv_scale is 0 (e.g. single point or flat object)
    if inv_scale == 0:
        inv_scale = torch.tensor(1.0, device=orig_xyz.device)
