...
def robust_normalize_and_align(points):
    """
    Centers points, scales them to fit within a unit sphere, 
    and aligns principal axes using PCA.
    """
    # 1. Center the points
    centroid = np.mean(points, axis=0)
    points = points - centroid
    
    # 2. Scale by the average distance (more robust to outliers than max_dist)
    avg_dist = np.mean(np.linalg.norm(points, axis=1))
    points /= avg_dist
    
    # 3. PCA Alignment: Orient the longest axis to X, second to Y, etc.
    # This helps ICP start from a much better 'initial guess'
    ...
    # Rotate points to the PCA frame
...
def evaluate_ply_vs_obj(pred_ply_path, gt_obj_path, num_points=10000, do_icp=True):
    """
    Compares a generated PLY against a Ground Truth OBJ.
    """
    print(f"\n--- Comparing Prediction (PLY) vs GT (OBJ) ---")
    print(f"Pred: {os.path.basename(pred_ply_path)}")
    print(f"GT:   {os.path.basename(gt_obj_path)}")
    
    # 1. Extraction
    pred_points = load_and_sample_ply(pred_ply_path, num_points=num_points)
    gt_points = load_and_sample_obj(gt_obj_path, num_points=num_points)
    ...
    # 3. Align (ICP) - Increased threshold slightly for different formats
    if do_icp:
        pred_aligned = apply_icp(pred_norm, gt_norm, threshold=0.1)
    else:
        pred_aligned = pred_norm
    
    # 4. Compute Metrics (Using your existing metric functions)
    f1, precision, recall = compute_f1_score(pred_aligned, gt_norm)
    iou = compute_voxel_iou(pred_aligned, gt_norm)
    dice = compute_voxel_dice(pred_aligned, gt_norm)
    chamfer = compute_chamfer_distance(pred_aligned, gt_norm)
    ...
    return {
        "File_ID": os.path.basename(gt_obj_path),
        "F1_Score": f1,
        "Precision": precision,
        "Recall": recall,
        "Voxel_IoU": iou,
...
def load_and_sample_obj(obj_path, num_points=10000):
    """Loads an OBJ file and samples points from the mesh surface."""
    ...
    try:
        ...
        if len(mesh.vertices) == 0:
            print(f"Warning: OBJ file {obj_path} has no vertices.")
            return np.zeros((0, 3))
...
if __name__ == "__main__":
    ...
    # gt_folder   = '/PHShome/yl535/project/python/datasets/AeroPath/lungs_segmented/'
    ...
    p = Path(os.path.join(gt_folder, 'obj_files', 'test'))
    ...
    for file_path in p.rglob('*.obj'):
        file_path = str(file_path)
        class_name = os.path.basename(os.path.dirname(file_path))
        file_name = os.path.basename(file_path).split(".")[0]

        pred_ply = os.path.join(pred_ply_folder, f'{file_name}.ply')
        if not os.path.exists(pred_ply):
            print(f"Prediction PLY not found for {file_name}, skipping...")
            continue
        gt_obj = file_path
        
        # Get metrics dictionary
        metrics_dict = evaluate_ply_vs_obj(pred_ply, gt_obj)
